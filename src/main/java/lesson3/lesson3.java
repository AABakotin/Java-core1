...
public class lesson3 {
    ...
    public static void main(String[] args) {
        ...
        //Третье задание
        int[] multiplicationNumbsInMassive = {1, 5, 3, 2, 11, 4, 5, 2, 4, 8, 9, 1};
        multiplicationMassive(multiplicationNumbsInMassive);
        ...
        //Шестое задание
        System.out.println(Arrays.toString(multiplicationNumbsInMassive));
        maxminMassive(multiplicationNumbsInMassive);
    ...
    private static void diagonal(int[][] quadArr) {
        for (int y = 0; y < quadArr.length; y++) {
            for (int x = 0; x < quadArr.length; x++) {
                if (y == x) quadArr[y][x] = 1;
                if (quadArr[x][y] == 0) {
                    quadArr[y][quadArr.length - y - 1] = 2;
                }
            }
            System.out.println(Arrays.toString(quadArr[y]));
